...
function getModifyMethod(e) {
  ...
  for (const i of modifyStatusesUrls)
    if (-1 < e.indexOf(i)) return "removeTimeLine";
...
function removeSearchTopic(e) {
  return (
    e.data &&
      ...
      e.data.trending_topic) &&
      delete e.data.trending_topic,
    e
  );
}
...
function isAd(e) {
  return (
    ...
    ("广告" == e.mblogtypename ||
      "热推" == e.mblogtypename ||
      "ad" == e.promotion?.type ||
      "ad" == e.page_info?.actionlog?.source ||
      "广告" == e.content_auth_info?.content_auth_title)
  );
}
...
function removeMainTab(e) {
  if (
    ...
    for (t of e.items)
      isAd(t.data) ||
        (t.data?.page_info?.video_limit && delete t.data.page_info.video_limit,
        t.data?.common_struct && delete t.data.common_struct,
        t.category &&
          "group" == t.category &&
          -1 == JSON.stringify(t.items).indexOf("profile_top")) ||
        a.push(t);
...
function removeMain(e) {
  if (
    ...
    for (t of e.items)
      "feed" == t.category
        ? isAd(t.data) || a.push(t)
        : "group" == t.category
        ? 0 < t.items.length &&
          t.items[0].data?.itemid?.includes("search_input")
          ? ((t.items = t.items.filter(
              (e) =>
                e?.data?.itemid?.includes("mine_topics") ||
                e?.data?.itemid?.includes("search_input") ||
                202 == e?.data?.card_type,
            )),
            (t.items[0].data.hotwords = [{ word: "搜索超话", tip: "" }]),
            a.push(t))
          : (0 < t.items.length &&
              t.items[0].data?.itemid?.includes("top_title")) ||
            (0 < t.items.length &&
              (t.items = Object.values(t.items).filter(
                (e) => "feed" === e.category || "card" === e.category,
              )),
            a.push(t))
        : (t.data?.card_type && -1 < [202, 200].indexOf(t.data.card_type)) ||
          a.push(t);
...
function removeSearchMain(e) {
  ...
  if (t) {
    ...
    for (a of t)
      a.payload &&
        (a.payload?.items && (a.payload.items = []),
        a.payload?.loadedInfo?.searchBarContent &&
          (a.payload.loadedInfo.searchBarContent = [{}]),
        a.payload?.loadedInfo?.headerBack &&
          (a.payload.loadedInfo.headerBack.channelStyleMap = {}),
        delete a.titleInfoAbsorb,
        delete a.titleInfo,
        delete a.title,
        o.push(a));
    (e.channelInfo.channels = o),
      e.header?.data && removeHeader(e.header.data),
      e.channelInfo?.moreChannels &&
        (delete e.channelInfo.moreChannels, delete e.channelInfo.channelConfig),
    ...
    for (t of e.items)
      "group" == t.category &&
        ((t.items = t.items
          .filter(
            (e) =>
              void 0 === e.data?.card_type ||
              101 === e.data?.card_type ||
              17 === e.data?.card_type,
          )
          .map((e) => (17 === e.data?.card_type && (e.data.col = 1), e))),
        0 < t.items.length) &&
        a.push(t);
...
function checkSearchWindow(e) {
  return (
    ...
    "card" == e.category &&
    ("finder_window" == e.data?.itemid ||
      "discover_gallery" == e.data?.itemid ||
      "more_frame" == e.data?.itemid ||
      208 == e.data?.card_type ||
      236 == e.data?.card_type ||
      247 == e.data?.card_type ||
      217 == e.data?.card_type ||
      101 == e.data?.card_type ||
      19 == e.data?.card_type ||
...
function removeSearch(e) {
  if (e.items) {
    ...
    for (t of e.items)
      "feed" == t.category
        ? isAd(t.data) ||
          (t.data?.page_info?.video_limit &&
            delete t.data.page_info.video_limit,
          a.push(t))
        : "group" == t.category
        ? ("guess" === t.header?.type && "search" != t.itemExt?.filterType) ||
          ((t.items = t.items
            .filter(
              (e) =>
                (void 0 === e.data?.card_type ||
                  17 === e.data?.card_type ||
                  10 === e.data?.card_type) &&
                "广告" !== e.data?.content_auth_info?.content_auth_title,
            )
            .map((e) => (17 === e.data?.card_type && (e.data.col = 1), e))),
          0 < t.items.length && a.push(t))
        : checkSearchWindow(t) || a.push(t);
    (e.items = a),
      e.loadedInfo &&
        ((e.loadedInfo.searchBarContent = []), e.loadedInfo.headerBack) &&
    ...
    for (t of e.items)
      "hotword" == t.itemId
        ? ((t.items = t.items.filter((e) =>
            e?.data?.pic?.includes("com/wb_search"),
          )),
          a.push(t))
        : "text" == t.type &&
          t?.text?.content?.includes("实时热点") &&
          a.push(t);
    (e.items = a),
      e.channelInfo && delete e.channelInfo,
      log("remove_search success");
  }
  return e;
}
...
function removeCards(e) {
  if ((e.hotwords && (e.hotwords = []), e.cards)) {
    var t,
      a = [];
    for (t of e.cards) {
      "232082type=1" != e.cardlistInfo?.containerid ||
        (17 != t.card_type && 58 != t.card_type && 11 != t.card_type) ||
        ((o = t.card_type + 1), (t = { card_type: o }));
      var o = t.card_group;
      if (o && 0 < o.length) {
        var i = [];
        for (const n of o)
          118 == n.card_type ||
            isAd(n.mblog) ||
            (-1 == JSON.stringify(n).indexOf("res_from:ads") && i.push(n));
        (t.card_group = i), a.push(t);
      } else {
        var r = t.card_type;
        -1 < [9, 165].indexOf(r)
          ? isAd(t.mblog) || a.push(t)
          : -1 < [1007, 180].indexOf(r) || a.push(t);
      }
    }
    e.cards = a;
  }
...
function removeTimeLine(e) {
  for (const a of ["ad", "advertises", "trends", "headers"])
    e[a] && delete e[a];
  if (e.statuses) {
    var t = [];
    for (const o of e.statuses)
      isAd(o) ||
        (lvZhouHandler(o),
        o.common_struct && delete o.common_struct,
        o.category && "group" == o.category) ||
        t.push(o);
    e.statuses = t;
  }
}
...
function itemExtendHandler(e) {
  ...
  (mainConfig.removeRelate || mainConfig.removeGood) &&
    e.trend &&
    e.trend.titles &&
    ((t = e.trend.titles.title),
    (mainConfig.removeRelate && "相关推荐" === t) ||
      (mainConfig.removeGood && "博主好物种草" === t)) &&
    delete e.trend,
  ...
  try {
    for (var t of e.items) {
      ...
      if ("mainnums_friends" === t.itemId)
        return (
          (a = t.click.modules[0].scheme),
          (t.click.modules[0].scheme = a.replace(
            "231093_-_selfrecomm",
  ...
  try {
    ...
    if (a) {
      ...
      for (var o of t.items)
        if (o.image)
          try {
            "alpha" != (dm = o.image.style.darkMode) &&
              (o.image.style.darkMode = "alpha"),
              (o.image.iconUrl = a[e++]),
              o.dot && (o.dot = []);
          } catch (e) {}
...
function removeHome(e) {
  if (e.items) {
    ...
    for (t of e.items) {
      var o = t.itemId;
      "profileme_mine" == o
        ? ((t = mainConfig.removeHomeVip ? removeHomeVip(t) : t).header
            ?.vipIcon && delete t.header.vipIcon,
          updateFollowOrder(t),
          a.push(t))
        : "100505_-_top8" == o
        ? (updateProfileSkin(t, "profileSkin1"), a.push(t))
        : "100505_-_newcreator" == o
        ? "grid" == t.type
          ? (updateProfileSkin(t, "profileSkin2"), a.push(t))
          : mainConfig.removeHomeCreatorTask || a.push(t)
        : ("100505_-_chaohua" != o &&
            "100505_-_manage" != o &&
            "100505_-_recentlyuser" != o) ||
          (0 < t.images?.length &&
            (t.images = t.images.filter(
              (e) =>
...
function containerHandler(e) {
  mainConfig.removeInterestFriendInTopic &&
    ...
    mainConfig.removeInterestTopic &&
      e.itemid &&
      (-1 < e.itemid.indexOf("infeed_may_interest_in")
        ? (log("remove 感兴趣的超话"), (e.card_group = []))
        : -1 < e.itemid.indexOf("infeed_friends_recommend") &&
...
function nextVideoHandler(e) {
  if (e.statuses) {
    ...
    for (t of e.statuses)
      if (!isAd(t)) {
        for (const o of [
          "forward_redpacket_info",
          "shopping",
          "float_info",
          "tags",
        ])
          t.video_info?.[o] && delete t.video_info[o];
        a.push(t);
      }
...
function tabSkinHandler(e) {
  try {
    var t,
      a = mainConfig.tabIconVersion;
    if (((e.data.canUse = 1), a && mainConfig.tabIconPath && !(a < 100))) {
      for (t of e.data.list)
        (t.version = a), (t.downloadlink = mainConfig.tabIconPath);
