(function () {
  ...
  if (
    typeof unsafeWindow.twitchAdSolutionsVersion !== "undefined" &&
    unsafeWindow.twitchAdSolutionsVersion >= ourTwitchAdSolutionsVersion
  ) {
    console.log(
      "skipping video-swap-new as there's another script active. ourVersion:" +
        ourTwitchAdSolutionsVersion +
        " activeVersion:" +
        unsafeWindow.twitchAdSolutionsVersion,
    );
    unsafeWindow.twitchAdSolutionsVersion = ourTwitchAdSolutionsVersion;
    return;
  }
  ...
  function getCleanWorker(worker) {
    ...
    while (proto) {
      ...
      if (
        workerStringConflicts.some((x) => workerString.includes(x)) &&
    ...
    var newWorker = class Worker extends getCleanWorker(unsafeWindow.Worker) {
      constructor(twitchBlobUrl, options) {
        ...
        var newBlobStr = `
                  ...
                  var workerString = getWasmWorkerJs('${twitchBlobUrl.replaceAll(
                    "'",
                    "%27",
                  )}');
                  declareOptions(self);
        ...
        this.addEventListener("message", (e) => {
          ...
          if (e.data.key == "UboShowAdBanner") {
            ...
            if (adDiv != null) {
              adDiv.P.textContent =
                "Blocking" + (e.data.isMidroll ? " midroll" : "") + " ads";
        ...
        function getAdDiv() {
          ...
          if (playerRootDiv != null) {
            ...
            if (adDiv == null) {
              ...
              adDiv.innerHTML =
                '<div class="player-ad-notice" style="color: white; background-color: rgba(0, 0, 0, 0.8); position: absolute; top: 0px; left: 0px; padding: 5px;"><p></p></div>';
              adDiv.style.display = "none";
  ...
  async function processM3U8(url, textStr, realFetch) {
        ...
        if (streamM3u8Response.status == 200) {
          ...
          if (streamM3u8 != null) {
            if (!streamM3u8.includes(AD_SIGNIFIER)) {
              console.log(
                "No more ads on main stream. Triggering player reload to go back to main stream...",
              );
              streamInfo.UseBackupStream = false;
              postMessage({ key: "UboHideAdBanner" });
              postMessage({ key: "UboReloadPlayer" });
            } else if (
              !streamM3u8.includes('"MIDROLL"') &&
              !streamM3u8.includes('"midroll"')
            ) {
    ...
    if (haveAdTags && streamInfo.BackupEncodings != null) {
      var streamM3u8Url =
        streamInfo.BackupEncodings.match(/^https:.*\.m3u8.*$/m)[0];
      var streamM3u8Response = await realFetch(streamM3u8Url);
      if (streamM3u8Response.status == 200) {
    ...
    fetch = async function (url, options) {
      if (typeof url === "string") {
        ...
        if (url.endsWith("m3u8")) {
          ...
          url.includes("/api/channel/hls/") &&
          !url.includes("picture-by-picture")
        ) {
          ...
          if (OPT_MODE_STRIP_AD_SEGMENTS) {
            return new Promise(async function (resolve, reject) {
              ...
              if (
                streamInfo != null &&
                streamInfo.Encodings != null &&
                (
                  await realFetch(
                    streamInfo.Encodings.match(/^https:.*\.m3u8$/m)[0],
                  )
                ).status !== 200
              ) {
              ...
              if (
                ...
                for (var i = 0; i < 2; i++) {
                  ...
                  var encodingsM3u8Response = await realFetch(
                    encodingsUrl,
                    options,
                  );
                  if (
                    encodingsM3u8Response != null &&
                    encodingsM3u8Response.status === 200
                  ) {
                    var encodingsM3u8 = await encodingsM3u8Response.text();
                    if (i == 0) {
                      streamInfo.Encodings = encodingsM3u8;
                      var streamM3u8Url =
                        encodingsM3u8.match(/^https:.*\.m3u8$/m)[0];
                      var streamM3u8Response = await realFetch(streamM3u8Url);
                      if (streamM3u8Response.status == 200) {
                        var streamM3u8 = await streamM3u8Response.text();
                        if (streamM3u8.includes(AD_SIGNIFIER)) {
  ...
  function getAccessToken(channelName, playerType, platform) {
    ...
    var body = null;
    var templateQuery =
      'query PlaybackAccessToken_Template($login: String!, $isLive: Boolean!, $vodID: ID!, $isVod: Boolean!, $playerType: String!) {  streamPlaybackAccessToken(channelName: $login, params: {platform: "' +
      platform +
      '", playerBackend: "mediaplayer", playerType: $playerType}) @include(if: $isLive) {    value    signature    __typename  }  videoPlaybackAccessToken(id: $vodID, params: {platform: "' +
      platform +
      '", playerBackend: "mediaplayer", playerType: $playerType}) @include(if: $isVod) {    value    signature    __typename  }}';
    body = {
      ...
      if (matches.length > 1) {
        ...
        for (let podPosition = 0; podPosition < podLength; podPosition++) {
          if (OPT_MODE_NOTIFY_ADS_WATCHED_MIN_REQUESTS) {
            ...
            await gqlRequest(
              makeGraphQlPacket("video_ad_pod_complete", radToken, baseData),
            );
          } else {
            const extendedData = {
              ...baseData,
            ...
            await gqlRequest(
              makeGraphQlPacket("video_ad_impression", radToken, extendedData),
            );
  ...
  async function handleWorkerFetchRequest(fetchRequest) {
    try {
      if (serverLikesThisBrowser || !serverHatesThisBrowser) {
        const response = await unsafeWindow.realFetch(
          fetchRequest.url,
      ...
      if (
        ...
        fetchRequest.options.headers["Sec-Ch-Ua"] =
        ...
        const responseObject = {
          ...
          headers: Object.fromEntries(
            parseHeaders(response.responseHeaders).entries(),
          ),
          body: responseBody,
        };
        return responseObject;
      ...
      throw {
        message:
          "Failed to resolve GQL request. Try the userscript version of the ad blocking solution",
      };
    } catch (error) {
      return {
        id: fetchRequest.id,
            ...
            if (OPT_ACCESS_TOKEN_PLAYER_TYPE) {
              ...
              if (Array.isArray(newBody)) {
                for (let i = 0; i < newBody.length; i++) {
                  newBody[i].variables.playerType =
                    OPT_ACCESS_TOKEN_PLAYER_TYPE;
                }
              } else {
  ...
  function reloadTwitchPlayer(isSeek, isPausePlay) {
    ...
    if (isSeek) {
      console.log(
        "Force seek to reset player (hopefully fixing any audio desync) pos:" +
          player.getPosition() +
          " range:" +
          JSON.stringify(player.getBuffered()),
      );
    ...
    if (player?.core?.state) {
      localStorage.setItem(
        lsKeyMuted,
        JSON.stringify({ default: player.core.state.muted }),
      );
    ...
    if (player?.core?.state?.quality?.group) {
      localStorage.setItem(
        lsKeyQuality,
        JSON.stringify({ default: player.core.state.quality.group }),
      );
    }
    playerState.setSrc({
  ...
  function onContentLoaded() {
    ...
    var visibilityChange = (e) => {
      if (typeof chrome !== "undefined") {
        ...
        if (videos.length > 0) {
          if (
            hidden.apply(document) === true ||
            (webkitHidden && webkitHidden.apply(document) === true)
          ) {
