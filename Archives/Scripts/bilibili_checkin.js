          ...
          if (body?.code === 0) {
            ...
            if (extra) {
              if (body.data.share) {
                ...
                config.share.num =
                  config.share.num == 0 ||
                  typeof config.share.num == "undefined"
                    ? 1
                    : config.share.num;
      ...
      if (aid !== 0) {
        ...
        const myRequest = {
          ...
          headers: {
            ...
            referer:
              "https://www.bilibili.com/video/BV1MT411G7fG?vd_source=1970993e2eff4af7be029aefcfa468b8",
            cookie: config.cookieStr,
          },
          body: $.queryStr(body),
        };
    ...
    (response) => {
      try {
        ...
        if (body?.code) {
          ...
          $.log(`- 距离满级还需: ${config.user.v6_day}天(登录 观看 分享)`);
