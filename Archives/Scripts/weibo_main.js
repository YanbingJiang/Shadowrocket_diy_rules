...
//主要的选项配置
const mainConfig = storeMainConfig
  ? JSON.parse(storeMainConfig)
  : {
...
//菜单配置
const itemMenusConfig = storeItemMenusConfig
  ? JSON.parse(storeItemMenusConfig)
  : {
      ...
      if (category == "group") {
        ...
        for (let sub of subItems) {
          ...
          if (
            !anchorId ||
            [
              "sg_bottom_tab_search_input",
              "multi_feed_entrance",
              "bottom_mix_activity",
              "cats_top_content",
              "chaohua_home_readpost_samecity_title",
              "chaohua_discovery_banner_1",
              "chaohua_home_readpost_samecity_content",
            ].indexOf(anchorId) == -1
          ) {
